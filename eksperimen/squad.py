...
# Path to input data
DATA_PATH = "combined_testsquad.json"  # Path to input dataset
...
print("\n✔ All QA pairs generated successfully!")
...
def process_qa_files(file_paths):
    ...
    for fp in tqdm(file_paths, desc="📂 Memroses file", dynamic_ncols=True):
        with open(fp) as f:
            dataset = json.load(f)

        model_key = os.path.basename(fp).split("_")[0]  # 'bert', 't5', …
        results.setdefault(model_key,
            {"data": [], "precision": [], "recall": [], "f1": []})

        # tqdm untuk tiap item dalam berkas
        for item in tqdm(dataset, desc=f"  → {model_key}", leave=False, dynamic_ncols=True):
            ...
            results[model_key]["data"].append({
                ...
                "bertscore": [list(t) for t in brs]    # tuple → list utk JSON
            })
...
for m, w in weights.items():
    print(f"  • {m:<10}: {w:.6f}")
print(f"\n📄  File bobot tersimpan di {weights_path}")
...
for model_name, model_results in all_results.items():          # ← pakai all_results
...
print(f"📊 Statistik BERTScore disimpan ke: {stats_file}")
...
def main():
    ...
    # ---------- Ringkasan ----------
    print("\n✅ Cosine similarity untuk semua model telah dihitung & disimpan.")
...
def process_and_save_to_json(grouped_data, model_label, output_path):
    ...
    print(f"✅ Vektor {model_label} berhasil disimpan ke {output_path}")

# ========== RUN ==========

print("📡 Memproses QA pairs dan menyimpan vektor ke file JSON...\n")
...
print("\n✅ Semua hasil QA vectors lengkap telah disimpan dalam format JSON.")
...
context_data = load_context_data("combined_testsquad.json")
...
def process_and_save_context_to_json(context_data, output_path):
    result = []
    for item in tqdm(context_data, desc="🔍 Memproses konteks"):
        title = item.get("title", "")
        context = item.get("context", "")
        if not context:
        ...
        try:
            ...
            print(f"❌ Gagal memproses: {title} — {e}")
            continue

    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, ensure_ascii=False)

    print(f"\n✅ Vektor konteks berhasil disimpan ke: {output_path}")

# ===============================
# RUN PROCESS
# ===============================
print("📡 Memproses konteks dan menyimpan vektor ke file JSON...\n")
...
print("\n✅ Semua hasil vektor konteks telah disimpan.")
...
def compute_final_vectors(input_path, output_path, weight, model_name):
    ...
    print(f"✅ Final vector {model_name} telah disimpan ke: {output_path}")
...
# Buat lookup: (title, context) ➜ context_vec
context_lookup = {
...
# ========================
# Ambil 31 QA terbaik berdasarkan similarity global
# ========================
top_k = 31
...
print(f"✅ Selesai! Total QA yang disimpan: {top_k} (dalam {len(final_output)} context group)")
...
def main():
    ...
    print(f"\n➡  File hasil QA+score   : {out_json}")
    print(f"➡  File statistik global : {stats_json}")
...
for k, v in stats.items():
    print(f"{k}: {v:.4f}")
print(f"\n✔️ Cosine similarity matrix saved to: {matrix_path}")
print(f"✔️ Stats saved to: {stats_path}")
...
def main():
    ...
    print(f"\n✅ Proses selesai! Hasil voting (berdasarkan context) disimpan di:\n{FINAL_QA_PATH}")
...
def main():
    ...
    print(f"✅ Skor BERTScore disimpan di: {out_json}")
    print(f"📊 Statistik BERTScore disimpan di: {stats_json}")
...
if __name__ == "__main__":
    ...
    print(f"✅ Saved {len(results_by_context)} items to {out_file}")
...
print("✅ Statistik BERTScore disimpan di 'output/attention/attention_bertscore_stats.json'")
