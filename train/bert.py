...
import os
import pandas as pd
# from sklearn.model_selection import train_test_split # Tidak lagi digunakan untuk splitting di main.py

from dataset import QGDataset # Memastikan QGDataset diimpor dari dataset.py
from trainer import Trainer # Memastikan Trainer diimpor dari trainer.py
...
# yang dapat digunakan untuk mengkonfigurasi proses pelatihan. Ini mencakup
# pengaturan hyperparameter seperti jumlah epoch, learning rate, ukuran batch,
# dan jalur ke model serta direktori penyimpanan. Perangkat (CPU/GPU) juga
# ditentukan secara otomatis atau dapat dispesifikasikan.
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--qg_model", type=str, default="mrm8488/bert2bert-multilingual_shared-question-generation")
    parser.add_argument("--pad_mask_id", type=int, default=-100)
    parser.add_argument("--save_dir", type=str, default="/home/samuel/tasi2425103/revisi/model/final_bert_2/")
...
# dari checkpoint yang diberikan. Ini juga menambahkan token khusus (<answer>, <context>)
# ke tokenizer, yang penting untuk format input model Question Generation.
...
# ============================ #
# Penjelasan: Fungsi ini memuat model Encoder-Decoder pra-terlatih dari checkpoint
# yang ditentukan. Ini mengkonfigurasi model, menonaktifkan cache (jika diperlukan),
# menyesuaikan ukuran embedding token agar sesuai dengan tokenizer yang diperluas
# (karena penambahan token khusus), dan mengaktifkan gradient checkpointing
# untuk penghematan memori. Model kemudian dipindahkan ke perangkat yang ditentukan (CPU/GPU).
...
# ============================ #
# Penjelasan: Fungsi ini telah dimodifikasi untuk memuat dataset training dan validasi
# dari file JSON yang sudah di-split (PISA dan SQuAD) secara terpisah, kemudian
# menggabungkannya. Ini menghilangkan kebutuhan untuk melakukan splitting di sini.
# Data kemudian dikonversi menjadi format `datasets.DatasetDict` yang digunakan oleh Trainer.
def load_and_combine_datasets(
    ...
    def _load_single_split(file_path: str):
        ...
        for item in raw_data:
            ...
            # --- PERBAIKAN DI SINI: Sertakan 'answer' ke dalam DataFrame ---
            ...
            data_list.append({
                ...
                "answer": answer_text # Sekarang kolom 'answer' disertakan!
            })
...
# menginisialisasi tokenizer dan model, memuat dan menggabungkan dataset
...
if __name__ == "__main__":
    ...
    # train_set dan valid_set sekarang akan dibuat dengan dataset yang memiliki kolom 'answer'
